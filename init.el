    ...
    (let ((errors-buffer (get-buffer-create "*Flyspell Errors*")))
      (with-current-buffer errors-buffer
        (let ((inhibit-read-only t))
          ...
          ;; Use buffer-local variables to store the source buffer
          (set (make-local-variable 'flyspell-source-buffer) source-buffer)
          ...
          (local-set-key (kbd "g") 
                         (lambda () 
                           (interactive)
                           (let ((buffer-to-check flyspell-source-buffer))
                             (kill-buffer)
                             (with-current-buffer buffer-to-check
                               (collect-flyspell-errors)))))
