...
(setq my/collect-flyspell-source-buffer nil)
    ...
    (let ((errors-buffer (get-buffer-create "*Flyspell Errors*")))
      (with-current-buffer errors-buffer
        (let ((inhibit-read-only t))
              ...
              (let ((pos start))
                (insert-button word
                               ...
                               'action (lambda (button)
                                         (let ((target-buffer (button-get button 'buffer))
          ...
          (insert (format "\nTotal: %d spelling errors found\n" (length error-list)))
          
          (goto-char (point-min))
          (text-mode)

          ;; Use the buffer-local variable for the refresh command
          (local-set-key (kbd "g") 
                         (lambda () 
                           (interactive)
                           (let ((buffer-to-check my/collect-flyspell-source-buffer))
                             (with-current-buffer buffer-to-check
                               (my/collect-flyspell-errors)))))
          
          (local-set-key (kbd "q") 'quit-window)))
      
      (switch-to-buffer-other-window errors-buffer)
      (message "Found %d flyspell errors" (length error-list)))))

;; For adding words directly from the source buffer
  ...
  (save-excursion
    (let ((current-word (thing-at-point 'word t)))
      (when current-word
        ...
        (ispell-send-string "#\n")
        (when ispell-process
          (accept-process-output ispell-process 0.2))
        (message "Added '%s' to personal dictionary" current-word)
        (pop-to-buffer my/collect-flyspell-source-buffer)
        (my/collect-flyspell-errors)))))

;; Bind this function to a key in text modes
(add-hook 'text-mode-hook
          (lambda ()
            (local-set-key (kbd "C-c d") 'my/flyspell-add-word-to-dict)))
