...
(define-key my-jump-keymap (kbd ";") #'my/fido-recentf)
(define-key my-jump-keymap (kbd "l") #'bookmark-jump)
(define-key my-jump-keymap (kbd "o") (lambda () (interactive) (switch-to-buffer "*scratch*")))
...
(define-key my-overrides-mode-map (kbd "M-H") #'my/window-swap-left)
(define-key my-overrides-mode-map (kbd "M-L") #'my/window-swap-right)
(define-key my-overrides-mode-map (kbd "M-K") #'my/window-swap-up)
(define-key my-overrides-mode-map (kbd "M-J") #'my/window-swap-down)
...
(define-key my-overrides-mode-map (kbd "C-M-h") (lambda () (interactive)
                              (my/adaptive-resize t 2)))
(define-key my-overrides-mode-map (kbd "C-M-j") (lambda () (interactive)
                              (my/adaptive-resize nil -1)))
(define-key my-overrides-mode-map (kbd "C-M-k") (lambda () (interactive)
                              (my/adaptive-resize nil 1)))
(global-set-key (kbd "C--") (lambda ()(interactive)(text-scale-adjust -1)))
(global-set-key (kbd "C-=") (lambda ()(interactive)(text-scale-adjust 1)))
(defun my/global-font-size-change (delta)
  "Change the default face height by DELTA steps on all frames.
DELTA is in 0.1pt units; updates future frames via `default-frame-alist'."
  (interactive "p")
  (let* ((step (* delta 10))
         (height (max 40 (+ (face-attribute 'default :height) step)))
         (font-spec (font-spec :family (face-attribute 'default :family)
                               :height height)))
    (set-face-attribute 'default nil :height height)
    (setq default-frame-alist
          (cons (cons 'font font-spec)
                (assq-delete-all 'font default-frame-alist)))))
(define-key my-overrides-mode-map (kbd "C-M--")
            (lambda () (interactive) (my/global-font-size-change -1)))
(define-key my-overrides-mode-map (kbd "C-M-=")
            (lambda () (interactive) (my/global-font-size-change 1)))
...
;; Accidental `C-x s' (instead of `C-x C-s') saves the current buffer
;; rather than prompting via `save-some-buffers'.  The overrides mode
;; map ensures this wins over mode-local bindings too.
(define-key my-overrides-mode-map (kbd "C-x s") #'save-buffer)
...
(defun my/tab-bar-push-window (direction)
  "Push the selected window's buffer to the tab DIRECTION tabs away.
DIRECTION is -1 for the left tab and 1 for the right tab."
  (interactive)
  (let* ((buf (window-buffer))
         (current (tab-bar--current-tab-index))
         (tabs (tab-bar-tabs))
         (target (mod (+ current direction) (length tabs))))
    (unless (eq target current)
      ...
      (if (one-window-p)
        ...
        (delete-window)))))
(define-key my-overrides-mode-map (kbd "M-s u")
            (lambda () (interactive) (my/tab-bar-push-window -1)))
(define-key my-overrides-mode-map (kbd "M-s i")
            (lambda () (interactive) (my/tab-bar-push-window 1)))
...
(define-key my-overrides-mode-map (kbd "C-M-n") #'(lambda ()(interactive)(scroll-up nil)))
(define-key my-overrides-mode-map (kbd "C-M-p") #'(lambda ()(interactive)(scroll-down nil)))
...
(defvar my/window-nav-passthrough-modes '(diff-mode)
  "Modes where M-n/M-p keep the mode's own commands.
The global window-navigation bindings stand aside in these modes so
that mode-provided bindings apply instead (e.g. `diff-hunk-next' /
`diff-hunk-prev' in `diff-mode').  Minibuffers always fall back to
history navigation; add other modes here to do the same there.")

(defun my/window-nav-passthrough-filter (binding)
  "Return BINDING for window navigation, except in passthrough contexts.
Used as a `menu-item' :filter for the global M-n/M-p window
bindings below.  Returning nil lets lookup fall through to the
buffer's own bindings:

- in the minibuffer, preserving M-p/M-n history navigation
  (`previous-history-element' / `next-history-element'); use C-x o
  to change windows from the minibuffer instead;
- in the modes listed in `my/window-nav-passthrough-modes',
  preserving their M-n/M-p commands (e.g. hunk navigation in
  `diff-mode')."
  (unless (or (minibufferp)
              (apply #'derived-mode-p my/window-nav-passthrough-modes))
    binding))

(define-key my-overrides-mode-map (kbd "M-n")
            '(menu-item "" other-window :filter my/window-nav-passthrough-filter))
(define-key my-overrides-mode-map (kbd "M-p")
            '(menu-item "" my/previous-window :filter my/window-nav-passthrough-filter))
...
(with-eval-after-load 'project
  (define-key project-prefix-map (kbd "V") #'my/vc-dir-here))
...
(custom-set-faces
 ...
 '(fixed-pitch ((t (:family "Monospace" :foundry "ADBO" :slant normal :weight regular :height 105 :width normal))))
...
(defun my/xsd-xref--local-activate ()
  "Register the XSD xref backend buffer-locally in nxml-mode, ahead
of eglot's buffer-local backend, so `M-.' keeps using exact
schema-index jumps even while a language server is connected."
  (add-hook 'xref-backend-functions #'my/xsd-xref-backend -100 t))

(add-hook 'nxml-mode-hook #'my/xsd-xref--local-activate)
...
;; -> ssh-agent-core
;;
(defconst my/ssh-auth-sock
  (let ((gcr     (format "/run/user/%d/gcr/ssh" (user-uid)))
        (keyring (format "/run/user/%d/keyring/ssh" (user-uid))))
    (cond ((file-exists-p gcr)     gcr)
          ((file-exists-p keyring) keyring)))
  "Discovered SSH agent socket for the current user, or nil.

When sway launches Emacs directly, SSH_AUTH_SOCK is not inherited, so
`git push' from vc-dir/magit cannot reach the gcr agent that already
holds the key and fails with
`ssh_askpass: exec(/usr/lib/ssh/ssh-askpass): No such file or directory'.")

(when (and my/ssh-auth-sock
           (not (getenv "SSH_AUTH_SOCK")))
  (setenv "SSH_AUTH_SOCK" my/ssh-auth-sock))

(unless (getenv "SSH_ASKPASS")
  (let ((askpass "/usr/bin/ksshaskpass"))
    (when (file-exists-p askpass)
      (setenv "SSH_ASKPASS" askpass)
      (unless (getenv "SSH_ASKPASS_REQUIRE")
