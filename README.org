...
Added =my/org-table-fixed-pitch= on =org-mode-hook=: a buffer-local face remap that makes the =org-table= face inherit =fixed-pitch=, so org tables keep their monospace grid alignment while =variable-pitch-mode= (=C-z p=) renders headings and prose in the variable font.  With variable pitch off the remap is visually a no-op.
