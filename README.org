...
On Windows, Emacs may lack the native =system-move-file-to-trash= function (e.g. MSYS2/Git-Portable builds), causing =move-file-to-trash= to fall back to the freedesktop.org method which uses =rename-file=. This fails with "Removing old name: Invalid argument" when trashing files on network drives (e.g. =z:/=) because =rename-file= cannot cross volumes.

This block defines a PowerShell-based =system-move-file-to-trash= that uses =Microsoft.VisualBasic.FileIO.FileSystem= — the same Shell API Windows Explorer uses. It correctly handles network drives (sends to Recycle Bin when available, deletes permanently when not) and works regardless of volume boundaries.
