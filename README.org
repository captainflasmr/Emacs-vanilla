...
Display the pixel dimensions and human-readable file size of the current image in the header line whenever an image is opened in =image-mode=. Falls back to an error message if the image metadata cannot be read.
