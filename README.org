...
Pressing =Z= in dired opens a transient menu showing compression formats with ratio hints:
...
Already-compressed files (matching =dired-compress-file-suffixes=) are automatically decompressed by =dired-compress=.  When multiple files are marked, they are bundled into a single archive (prompts for archive name, defaulting to the directory name).  Single files and directories are compressed individually.  Formats without a single-file command (tar.gz, 7z) always use the archive path.  All compression runs asynchronously via =start-file-process= with a sentinel that reverts the dired buffer on completion.  The dired header-line shows =● Compressing xz... ●= during compression and =● ✓ Compressed: filename (1.2M) ●= on completion (clears after 6 seconds).
