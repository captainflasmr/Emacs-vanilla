...
Added =my/set-divider-color= on =C-z D=: prompts for a colour (same =read-color= picker as =C-z u= / =C-z U= / =C-z B=) and recolours the three =window-divider= faces (divider plus first/last pixel) on the selected frame, or all frames with a prefix argument.  Pairs with =C-z d=, which toggles the dividing line itself.

Added =my/org-fixed-pitch= on =org-mode-hook=: buffer-local face remaps that make the =org-table= face and the source block faces (=org-block=, =org-block-begin-line=, =org-block-end-line=) plus inline code (=org-code=, =org-inline-src-block=) inherit =fixed-pitch=, so org tables and code keep their monospace alignment while =variable-pitch-mode= (=C-z p=) renders headings and prose in the variable font.  With variable pitch off the remaps are visually a no-op.

=my/kill-ring-save= (=M-w=, =C-c w=) now copies only the visible text of the region when called with a prefix argument, via =org-copy-visible=, so folded headings and hidden blocks are skipped instead of being copied in full.

** <2026-09-05>

Magit-style branch and worktree first actions for vc and vc-dir

- New worktree commands in =vc-git-worktree-core=: =my/vc-git-worktree-checkout= checks out an existing branch in a new worktree, =my/vc-git-worktree-branch= creates a new branch from a start point in a new worktree (Magit's two most common first actions, both defaulting to a sibling directory named <project>-<branch>), and =my/vc-git-worktree-move= relocates a worktree; =my/vc-git-worktree-add= now shares the same internal worker
- New =vc-git-branch-core= section filling the gaps around stock =C-x v b s= (switch), =b c= (create) and =b l= (log): =my/vc-git-merge-branch= (plain merge, squash or ff-only), =my/vc-git-rename-branch= (works on the checked-out branch), =my/vc-git-delete-branch= (prefix forces with =-D=, checked-out branch protected), and =my/vc-git-branch-list= showing current mark, upstream and subject with =RET= checking out
- In =vc-dir=: =Z a=, =Z b=, =Z c=, =Z g=, =Z k=, =Z l=, =Z m=, =Z p= for worktrees and =b b=, =b m=, =b M=, =b k=, =b B= for branches (sharing stock's lowercase =b= prefix); globally under =C-x v z …= and =C-x v b …=
- Emacs 31 alignment: our branch list moved =b L= → =b B= (31 binds =b L= to =vc-print-root-branch-log=); the rest of our =Z= and =b= maps are collision-free against 31's new =C-x v w= working-tree commands, whose differing semantics (=w k= kills buffers rather than removing the tree, =w s= switches projects rather than opening =vc-dir=) are exactly why the Magit-style =Z= prefix stays
- Key prefixes now match Magit for muscle memory, prompted by Bozhidar Batsov's [[https://emacsredux.com/blog/2026/09/02/working-with-git-worktrees-in-magit/][Working with Git Worktrees in Magit]]: worktrees moved =W …= → =Z …= with Magit's own =b=/=c= semantics (=Z b= checks out an existing branch, =Z c= creates a new one), =W s= → =Z g= like =magit-worktree-status=, and the branch commands merged into stock's lowercase =b= prefix (list on =b B=); the DIYer header refresh returns to plain =B=
- =my/vc-git-merge-branch= merges a branch into the current one as a plain merge, squash (staged for a later commit) or fast-forward-only; stock =C-x v m= is file-oriented and a verified no-op for git branch merges, so merging gets its own command on =b M= (=C-x v b M=)
- Branch keys audited against Magit's branch transient source (=magit-branch.el=): rename =b r= → =b m=, merge =b m= → =b M= (bare =m= means mark in =vc-dir=), switch gains a =b b= checkout alias; =b s= and =b l= stay stock (spin-off and checkout-local have no equivalents here), =b x= left free

** <2026-09-04>

Add git worktree lifecycle support to vc and vc-dir

- New =vc-git-worktree-core= section: built-in =vc= already tracks files inside a worktree, but had no commands to create, switch, remove, prune or list worktrees
- =my/vc-git-worktree-add= creates a worktree at a prompted location (defaulting to a sibling directory named after the branch) and opens =vc-dir= there; empty branch checks out =HEAD=, prefix argument creates a new branch from a prompted start point
- =my/vc-git-worktree-switch= selects a worktree with branch annotations and opens =vc-dir=; =my/vc-git-worktree-remove= deletes one after confirmation (prefix forces, and a =vc-dir= sitting inside the removed tree falls back to the main root); =my/vc-git-worktree-prune= clears stale metadata
- =my/vc-git-worktree-list= shows all worktrees (branch, short HEAD, bare/detached/locked/prunable flags parsed from =git worktree list --porcelain=) with =RET= jumping to =vc-dir=
- In =vc-dir=: =W a=, =W s=, =W r= (=W k=), =W p=, =W l=; globally: =C-x v w a=, =w s=, =w r=, =w k=, =w p=, =w l=

** <2026-08-24>

Add global C-x T and C-x E bindings for terminal and file manager

- =C-x T= now opens a new native terminal window in the current buffer's directory from any buffer (reuses the existing =my/dired-open-terminal= detection instead of just =C-c t= in dired)
- =C-x E= reveals the current buffer's file in the native file manager via =my/open-file-manager=: a directory opens the manager at it, a file shows its parent folder (Explorer on Windows, Finder via =open -R= on macOS, and on Linux/BSD the first installed of nautilus, thunar, dolphin --select, nemo, caja, pcmanfm, pcmanfm-qt, falling back to =xdg-open=)

** <2026-06-26>

Add vc-dir integration for git skip-worktree

- New =vc-git-skip-worktree-core= section exposing =git update-index --skip-worktree= so app-rewritten files (e.g. =~/.config/digikamrc=) can be kept tracked without dirtying the tree
- =my/vc-git-skip-worktree-p= queries =git ls-files -v= for the =S = flag (with =point-min= reset so the match fires correctly)
- In =vc-dir=: =* s= toggles the bit on the file at point, =* u= clears it on a single file, =* U= clears every flagged file in the repo after a count prompt
- Outside =vc-dir=: =M-x my/vc-git-toggle-skip-worktree= and =M-x my/vc-git-clear-skip-worktree= act on the current buffer's file
- =vc-git-dir-status-files= is advised to inject a =skip-worktree= state entry for each flagged file (fetched once via =git ls-files -v -z=), since git otherwise reports them as clean and =vc-dir= would hide them
- =vc-dir-printer= is advised to append a warning-coloured ={S}= suffix to those rows

** <2026-06-12>

Async decompression in dired to match async compression

- Decompressing via =z= (when marked files are already compressed) now runs asynchronously with the same header-line progress feedback as compression (=● Decompressing... ●= then =● ✓ Decompressed: filename ●=)
- Added =my/dired-decompress-formats=, =my/dired-decompress-file=, and =my/dired-decompress=; dispatches on extension (tar.*, gz, xz, bz2, zst, lz, 7z, zip)
- =my/dired-do-compress= now routes the decompress branch through =my/dired-decompress= instead of the synchronous =dired-do-compress=
- Generalised =my/dired-compress-sentinel= via a =my-action= process property so it reports "Compressed"/"Decompressed" appropriately

** <2026-05-13>

Add header-line progress feedback for dired compression and gh-release operations

- Dired compression shows =● Compressing xz... ●= in the header-line during async compression, then =● ✓ Compressed: filename (1.2M) ●= on completion (clears after 6 seconds)
- =my/gh-release-create= and =my/gh-release-download= show header-line progress in the releases buffer (creating, ✓ created, ✗ failed with error reason)
- =my/gh-release-list= now formats tab-separated =gh release list= output into aligned columns
- =my/gh-release-create= file default now shows full dired path instead of basename
- Suppressed distracting mu4e indexing progress messages from the minibuffer

** <2026-04-26>

Enhanced dired compression with transient format menu and async execution

- Replaced default =dired-do-compress= (=Z=) with a transient menu showing compression format and ratio hints
- Added async compression via =start-file-process= with a sentinel that reverts the dired buffer on completion
- Supported formats: xz, gzip, tar.gz, bzip2, zstd, lzip, 7z -- each with single-file and directory/archive paths
- Directories are compressed as tar archives piped through the chosen compressor (or =tar -czf= for tar.gz, =7z a= for 7z)
- Already-compressed files are automatically decompressed via =dired-compress=
- Added =my/dired-compress-transient=, =my/dired-compress-with=, =my/dired-compress-directory=, =my/dired-compress-file=, and =my/dired-compress-sentinel=
- Generated individual commands (=my/dired-compress-xz=, =my/dired-compress-gzip=, etc.) via =my/dired-define-compress-command= macro
- Bound =z= in dired to =my/dired-do-compress=; if any marked file is already compressed, decompresses directly via =dired-do-compress= instead of showing the transient
...
Redirect all Customize writes to a temporary file so Emacs never writes =custom-set-faces= or =custom-set-variables= into init.el. The temp file is never loaded, so those settings are effectively discarded.
...
Infrastructure for global keybinding overrides that take precedence over major mode maps.
...
Whole-buffer paragraph filling. =C-x M-q= fills every paragraph in the buffer at the current =fill-column= — set the column (e.g. to 999 to collapse each paragraph onto a single line) and fill the whole document in one keystroke. Unlike =M-q= (=fill-paragraph=, one paragraph) this uses =fill-individual-paragraphs=, so each change of indentation starts a new paragraph and keeps its own indentation.
In Org buffers the literal blocks (=src=, =example=, =export=,
=verbatim=, =comment=, fixed-width) are protected: their contents are
left untouched and only the surrounding paragraphs are filled.  List
items are filled too — each item is an Org paragraph element, so a
wrapped item's continuation lines are joined while the items themselves
stay separate.
...
Container management UI for Docker and Podman.  Provides tabulated list views for containers, images, and volumes, plus a transient menu for compose operations and container actions.  Inspired by emacs-solo-container.

Keybindings in list buffers: =n/p= navigate, =m= mark, =M= unmark all, =s/S/r= stop/start/restart, =D= remove, =o= logs, =e= shell, =w= copy ID, =g= refresh, =i= images, =v= volumes, =?= menu.
...
[[https://git-scm.com/docs/git-update-index#Documentation/git-update-index.txt---skip-worktree][git update-index --skip-worktree]] tells git to treat the working-tree copy of a file as authoritative: =git status=, =git add= and =git diff= ignore local changes to that path, but the committed snapshot stays in the repo. This is perfect for files that external applications keep rewriting on every launch (e.g. =~/.config/digikamrc=) where you want a baseline committed but don't want the app to keep dirtying your tree. It is a local index flag, so a fresh clone will not have it set.

From a =vc-dir= buffer: place point on a file and press =* s= to toggle the bit. Because git reports a skip-worktree file as clean, =vc-dir= would normally hide it altogether — the toggle would seem to disappear. To fix this, the listing is post-processed to inject a =skip-worktree= entry for every flagged file under the =vc-dir= directory (fetched once via =git ls-files -v -z=), and the printer appends a warning-coloured ={S}= suffix to those rows. Press =g= to refresh after changes. Outside =vc-dir=, =M-x my/vc-git-toggle-skip-worktree= acts on the current buffer's file. Only operates on git-tracked files (=vc-backend= = Git).
...
Configures the tab bar to display uniform, small, fixed-width tabs that don't autofit. Sets =tab-bar-auto-width= to nil to prevent tabs from auto-resizing to fill the bar, and defines a custom format function that truncates and right-pads each tab name to =tab-bar-tab-name-truncated-max= so every tab is the same width. Tweak =tab-bar-tab-name-truncated-max= to change the tab width.
...
Associates =.jsonc= files (JSON with comments, used by VS Code and many modern tools) with Emacs's built-in =js-json-mode=, which provides syntax highlighting and indentation for JSON. No package is required: =js-json-mode= is built in and autoloaded since Emacs 27.1.
...
Forces UTF-8 as the default coding system everywhere. On GNU/Linux the UTF-8 locale makes this the default automatically, but Windows falls back to =iso-latin-1=, which cannot encode characters such as curly quotes (U+2018/U+2019) and so prompts for a coding system on every save.
...
Moves files to the system trash asynchronously via rsync (Linux) or PowerShell (Windows), with header-line progress. On Linux, the freedesktop.org trash spec is followed by writing a =.trashinfo= sidecar synchronously (tiny, instant) and then moving directories atomically with =mv= or transferring individual files with rsync =--remove-source-files= in the background. On Windows, PowerShell is used asynchronously via =start-process=.

When =my/async-trash-enabled= is =nil= (the default), the async mechanism is bypassed and Emacs' built-in blocking deletion is used instead (which still honours =delete-by-moving-to-trash= on supported platforms). Set it to =t= to enable the async rsync/PowerShell path:

#+begin_src elisp :tangle no
(setq my/async-trash-enabled t)
#+end_src
...
Emacs' built-in =vc= knows how to create tags (=C-x v s=) but the feature is invisible in =vc-dir= and there is no way to push a tag (or to make an annotated tag) at all. This section adds git tag support mirroring how =vc-create-branch= behaves for branches: from a =vc-dir= buffer press =t= to create a tag at the tip of the current branch (plain =t= makes an annotated tag and prompts for a message; =C-u t= makes a lightweight tag) and =T= to push an existing tag to a remote, completing over =git tag= and =git remote= output with =origin= as the default remote. The same create binding is available globally as =C-x v t= from any file buffer.
...
Built-in =vc= tracks files inside a Git worktree automatically (the =.git= pointer file resolves transparently, so diffs, commits and =vc-dir= just work per worktree) but offers no commands to manage the worktree lifecycle itself. This section adds worktree support mirroring the tag helpers above, deliberately keyed like Magit's worktree map (see Bozhidar Batsov's [[https://emacsredux.com/blog/2026/09/02/working-with-git-worktrees-in-magit/][Working with Git Worktrees in Magit]]): from a =vc-dir= buffer press =Z b= to check out an existing branch in a new worktree and =Z c= to create a new branch in a new worktree (the two Magit-style first actions, matching =magit-worktree-checkout= and =magit-worktree-branch= key for key, each defaulting the location to a sibling directory named <project>-<branch>), =Z a= for the general case (empty input checks out =HEAD=; =C-u Z a= creates a new branch from a prompted start point instead), =Z g= to jump to another worktree's =vc-dir= like =magit-worktree-status=, =Z m= to move a worktree and =Z k= to remove one after confirmation (prefix forces) exactly as in Magit, plus =Z l= to list all worktrees in a buffer where =RET= jumps to =vc-dir= and =Z p= to prune stale metadata, two extras with no Magit equivalent. A =vc-dir= sitting inside a removed tree falls back to the main root rather than dying. The same commands are available globally as =C-x v z a=, =z b=, =z c=, =z g=, =z m=, =z k=, =z l= and =z p= from any file buffer.
...
Stock =vc= already covers switching (=C-x v b s=), creating (=C-x v b c=) and logging (=C-x v b l=) branches, and this section hangs the rest on that very same lowercase =b= prefix — audited key-for-key against Magit's own branch transient, so =b b= checks out (aliasing stock's =b s= switch), =b m= renames exactly like =magit-branch-rename= (git's own =-m= flag), and =b k= deletes exactly like =magit-branch-delete=. Two Magit letters mean something else here and are deliberately left alone: =b s= stays stock switch (Magit’s is spin-off, which has no equivalent) and =b l= stays stock log (Magit’s checks out the local branch). Merge has no slot under Magit’s =b= either — over there it lives on the top-level =m= transient, but in =vc-dir= bare =m= means mark — so it takes the nearest free key =b M=. (Note =C-x v m= itself is file-oriented and a no-op for git branch merges, so merging gets its own command regardless.) From a =vc-dir= buffer: =b m= renames a branch (defaulting to the current one; renaming the checked-out branch works); =b M= merges (plain commit, squash staged for later, or fast-forward-only); =b k= deletes after confirmation (prefix forces =-D=; the checked-out branch is protected, switch away first); =b B= lists local branches with the current branch marked, upstream and subject shown, where =RET= checks out the branch at point (capital =B= for Branches; =b L= belongs to 31's =vc-print-root-branch-log=). The same commands are available globally as =C-x v b b=, =b m=, =b M=, =b k= and =b B= from any file buffer.
...
Native source-block fontification is enabled explicitly.  Large literate files can leave a newly visited source block without a font-lock text property, so a buffer-local post-command/scroll hook asks Emacs to ensure fontification for the visible range or current source block.  This avoids running the much more expensive full-buffer =font-lock-update= after every jump.
...
Enhances Dired, the directory editor, with async compression via a transient format menu, plus additional file management workflows.

Pressing =z= in dired opens a transient menu showing compression and GPG encryption options:

#+begin_example
Compress or encrypt <files>
z  xz         – best ratio, slow
g  gzip       – fast, decent ratio
t  tar.gz     – fast, decent ratio
b  bzip2      – good ratio, medium
s  zstd       – fast, good ratio
l  lzip       – best ratio, slow
7  7z         – best ratio, slow
e  gpg    – symmetric encrypt
c  gpg    – tar.gz + encrypt
#+end_example

Already-compressed files (matching =dired-compress-file-suffixes=) are automatically decompressed by =dired-compress=, and marked =.gpg= files are decrypted via =my/dired-decrypt=.  When multiple files are marked, they are bundled into a single archive (prompts for archive name, defaulting to the directory name).  Single files and directories are compressed individually.  Formats without a single-file command (tar.gz, 7z) always use the archive path.  All compression runs asynchronously via =start-file-process= with a sentinel that reverts the dired buffer on completion.  The dired header-line shows =● Compressing xz... ●= during compression and =● ✓ Compressed: filename (1.2M) ●= on completion (clears after 6 seconds).

GPG encryption works the same way: =e= encrypts each marked file symmetrically to =file.gpg= and =c= tars the selection up and encrypts it to =dir.tar.gz.gpg= (AES256).  The passphrase is read once per operation with =read-passwd= (asked twice when encrypting) and piped to =gpg --pinentry-mode loopback --passphrase-fd 0=, so no external pinentry is needed; the header-line reports =● Encrypting filename... ●= and =● ✓ Encrypted: filename (12M) ●=.  Originals are kept, both when encrypting and when decrypting.

Pressing =C-c t= in dired opens the current directory in an external terminal.  Terminal detection honors =$TERMINAL=, then XDG hints (=xdg-terminal-exec=, =XDG_CURRENT_DESKTOP=), then a list of common emulators via =executable-find=.  Windows (wt, powershell, cmd) and macOS (Terminal.app) are supported.
...
Global access to the external terminal and the native file manager from the current buffer's location.  =C-x T= opens a new native terminal window in the current buffer's directory (the same =my/dired-open-terminal= used for =C-c t= in dired, but bound globally).  =C-x E= reveals the current buffer's file in the native file manager.
...
Custom xref backend for XSD schemas: ~xref-find-definitions~ (M-.) jumps from ~type=~, ~base=~ and ~ref=~ attribute values to the global ~simpleType~ / ~complexType~ / ~element~ / ~attribute~ / ~attributeGroup~ / ~group~ declaration with that name. The current file is searched first, then sibling ~.xsd~ files in the same directory; a prefixed reference like ~lox:FooType~ only matches files whose ~targetNamespace~ matches the prefix's binding, so cross-schema jumps land in the right file. ~xref-find-references~ (M-?) lists every occurrence of a name.

No external tools required: indexes are built with ~libxml-parse-xml-region~ (regex scanning as a fallback), cached per file by mtime and per buffer by modification tick, and the backend is registered ahead of dumb-jump so it wins in XML buffers. W3C builtins like ~xs:string~ correctly resolve to nothing.
...
Display the original pixel dimensions and human-readable file size of the current image in the header line whenever an image is opened in =image-mode=. Uses a separate image created with =:scale 1= to obtain the native (unscaled) dimensions, independent of any window-fitting scaling applied by =image-mode=. When the display size differs from the original, both are shown. Falls back to an error message if the image metadata cannot be read.
...
GitHub release management using the =gh= CLI. Provides listing, creating, deleting, viewing, downloading, and editing releases from within Emacs without any external packages.  Release create/download show header-line progress in the releases buffer.  Release list output is formatted into aligned columns.
