...
In Org buffers the literal blocks (=src=, =example=, =export=, =verbatim=, =comment=, fixed-width) are protected: their contents are left untouched and only the surrounding paragraphs are filled.
