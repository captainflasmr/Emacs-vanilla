...
- In =vc-dir=: =W c=, =W b=, =W m= for worktrees and =B m=, =B r=, =B d= (=B k=), =B l= for branches; globally under =C-x v w …= and =C-x v B …=
- =B= in =vc-dir= is now a branch prefix, so the DIYer branches-and-tags header refresh (which also runs automatically on its hook) moves to =B B=
...
Built-in =vc= tracks files inside a Git worktree automatically (the =.git= pointer file resolves transparently, so diffs, commits and =vc-dir= just work per worktree) but offers no commands to manage the worktree lifecycle itself. This section adds worktree support mirroring the tag helpers above: from a =vc-dir= buffer press =W a= to create a worktree at a prompted location checking out an existing branch (empty input checks out =HEAD=; =C-u W a= creates a new branch from a prompted start point instead, defaulting the location to a sibling directory named <project>-<branch>), =W c= to check out an existing branch in a new worktree and =W b= to create a new branch in a new worktree (the two Magit-style first actions, each defaulting the location the same way), =W m= to move a worktree, =W s= to switch to another worktree in =vc-dir= with branch annotations, =W r= (=W k=) to remove one after confirmation (prefix forces), =W p= to prune stale metadata, and =W l= to list all worktrees in a buffer where =RET= jumps to =vc-dir=. The same commands are available globally as =C-x v w a=, =w c=, =w b=, =w m=, =w s=, =w r=, =w k=, =w p= and =w l= from any file buffer.
...
Stock =vc= already covers switching (=C-x v b s=), creating (=C-x v b c=) and logging (=C-x v b l=) branches, so this section only fills the remaining Magit-style gaps: merging, renaming, deleting and listing. (Note =C-x v m= is file-oriented and a no-op for git branch merges, so merging gets its own command here.) From a =vc-dir= buffer press =B m= to merge a branch into the current one, choosing a plain merge commit, a squash staged for a later commit, or fast-forward-only; =B r= to rename a branch (defaulting to the current one; renaming the checked-out branch works), =B d= (=B k=) to delete one after confirmation (prefix forces with =-D=; the checked-out branch is protected, switch away first), and =B l= to list local branches with the current branch marked, upstream and subject shown, where =RET= checks out the branch at point. The same commands are available globally as =C-x v B m=, =C-x v B r=, =B d=, =B k= and =B l= from any file buffer.
