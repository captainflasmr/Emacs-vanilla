...
Stock =vc= already covers switching (=C-x v b s=), creating (=C-x v b c=) and logging (=C-x v b l=) branches, and this section hangs the rest on that very same lowercase =b= prefix — audited key-for-key against Magit's own branch transient, so =b b= checks out (aliasing stock's =b s= switch), =b m= renames exactly like =magit-branch-rename= (git's own =-m= flag), and =b k= deletes exactly like =magit-branch-delete=. Two Magit letters mean something else here and are deliberately left alone: =b s= stays stock switch (Magit’s is spin-off, which has no equivalent) and =b l= stays stock log (Magit’s checks out the local branch). Merge has no slot under Magit’s =b= either — over there it lives on the top-level =m= transient, but in =vc-dir= bare =m= means mark — so it takes the nearest free key =b M=. (Note =C-x v m= itself is file-oriented and a no-op for git branch merges, so merging gets its own command regardless.) From a =vc-dir= buffer: =b m= renames a branch (defaulting to the current one; renaming the checked-out branch works); =b M= merges (plain commit, squash staged for later, or fast-forward-only); =b k= deletes after confirmation (prefix forces =-D=; the checked-out branch is protected, switch away first); =b L= lists local branches with the current branch marked, upstream and subject shown, where =RET= checks out the branch at point (capital =L=, since stock owns =b l=). The same commands are available globally as =C-x v b b=, =b m=, =b M=, =b k= and =b L= from any file buffer.
